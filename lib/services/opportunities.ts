...
export const opportunityService = {
  ...
  async getStats(companyId: string) {
    ...
    const highMatch = data.filter(o => o.match_score >= 80).length;
    const converted = data.filter(o => o.internal_status.startsWith('converted_')).length;

    const last24h = new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString();
    const newLastSync = data.filter(o => o.created_at >= last24h).length;

    const soon = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString();
    const expiringSoon = data.filter(o =>
      o.opening_date &&
      o.opening_date <= soon &&
      o.opening_date >= new Date().toISOString()
    ).length;
