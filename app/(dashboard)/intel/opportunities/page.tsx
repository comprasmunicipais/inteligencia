...
export default function OpportunitiesPage() {
  ...
  const [stats, setStats] = useState({ total: 0, newLastSync: 0, highMatch: 0, expiringSoon: 0, converted: 0 });
  ...
  const [filters, setFilters] = useState({ location: '', modality: '', minScore: 0 });
  ...
  const handleSaveProposal = async () => {
    ...
    } catch { toast.error('Erro ao salvar proposta.'); }
  ...
  return (
    <>
      <Header title="Licitações Abertas e Publicadas" subtitle="Monitoramento em tempo real de oportunidades públicas com destaque para aderência ao perfil da empresa." />

      <div className="flex-1 overflow-y-auto p-8 bg-[#f8fafc]">
        {loading ? (
          <div className="flex justify-center py-12"><Loader2 className="size-8 text-[#0f49bd] animate-spin" /></div>
        ) : (
          <div className="max-w-7xl mx-auto space-y-6">
            ...
            <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm flex flex-col md:flex-row gap-4 items-center">
              <div className="relative flex-1 w-full">
                ...
                <input type="text" placeholder="Buscar por objeto, órgão ou palavra-chave..." className="w-full pl-10 pr-4 py-2.5 border border-gray-200 rounded-lg focus:ring-2 focus:ring-[#0f49bd]/20 focus:border-[#0f49bd] outline-none transition-all text-sm" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
              </div>
              <div className="flex items-center gap-2 w-full md:w-auto">
                <button onClick={handleRecalculateScores} disabled={recalculating} className="flex-1 md:flex-none flex items-center justify-center gap-2 px-4 py-2.5 border border-gray-200 rounded-lg text-sm font-bold text-gray-700 hover:bg-gray-50 transition-colors disabled:opacity-50">
                  <Zap className={cn('size-4', recalculating && 'animate-pulse text-yellow-500')} />
                  {recalculating ? 'Recalculando...' : 'Recalcular Score'}
                </button>
                <button onClick={handleSync} disabled={syncing} className="flex-1 md:flex-none flex items-center justify-center gap-2 px-4 py-2.5 border border-gray-200 rounded-lg text-sm font-bold text-gray-700 hover:bg-gray-50 transition-colors disabled:opacity-50">
                  <RefreshCw className={cn('size-4', syncing && 'animate-spin')} />
                  Sincronizar
                </button>
                <button onClick={() => setIsFilterModalOpen(true)} className="flex-1 md:flex-none flex items-center justify-center gap-2 px-4 py-2.5 border border-gray-200 rounded-lg text-sm font-bold text-gray-700 hover:bg-gray-50 transition-colors">
                  <Filter className="size-4" /> Filtros
                </button>
                <button onClick={() => setIsIAInsightsModalOpen(true)} className="flex-1 md:flex-none flex items-center justify-center gap-2 px-4 py-2.5 bg-[#0f49bd] text-white rounded-lg text-sm font-bold hover:bg-[#0a3690] transition-colors shadow-sm">
            ...
            <div className="flex items-center gap-1 border-b border-gray-200">
              {[{ key: 'all', label: 'Todas' }, { key: 'new', label: 'Novas' }, { key: 'under_review', label: 'Em Revisão' }, { key: 'relevant', label: 'Relevantes' }, { key: 'discarded', label: 'Descartadas' }].map((tab) => (
                <button key={tab.key} onClick={() => setActiveTab(tab.key as TabKey)} className={cn('px-6 py-3 text-sm font-bold transition-all border-b-2', activeTab === tab.key ? 'border-[#0f49bd] text-[#0f49bd]' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300')}>
                  {tab.label}
                </button>
              ))}
            </div>
            ...
            {filteredOpps.length === 0 ? (
              <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-8">
                <EmptyState icon={AlertCircle} title="Nenhuma oportunidade encontrada" description="Não há registros compatíveis com os filtros aplicados." />
              </div>
            ) : (
              <div className="grid grid-cols-1 gap-4">
                {filteredOpps.map((opp) => (
                  <div key={opp.id} className="bg-white rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-all overflow-hidden group">
                    <div className="p-6 flex flex-col md:flex-row gap-6">
                      <div className="flex-1 space-y-4">
                        <div className="flex items-start justify-between gap-4">
                          <div>
                            <div className="flex items-center gap-2 mb-2 flex-wrap">
                              <span className="bg-blue-50 text-blue-700 text-[10px] font-bold px-2 py-0.5 rounded uppercase tracking-wider border border-blue-100">Licitação</span>
                            ...
                            <h3 className="text-lg font-bold text-gray-900 group-hover:text-[#0f49bd] transition-colors">{opp.title}</h3>
                          ...
                          <div className="flex flex-col items-end">
                            <div className={cn('size-12 rounded-full border-4 flex items-center justify-center text-sm font-bold', Number(opp.match_score || 0) >= 90 ? 'border-green-100 text-green-600 bg-green-50' : Number(opp.match_score || 0) >= 70 ? 'border-blue-100 text-blue-600 bg-blue-50' : Number(opp.match_score || 0) >= 50 ? 'border-amber-100 text-amber-600 bg-amber-50' : 'border-gray-100 text-gray-400 bg-gray-50')}>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {!!opp.modality && <span className="bg-gray-100 text-gray-600 text-[10px] font-bold px-2 py-1 rounded">{opp.modality}</span>}
                          {!!opp.situation && <span className="bg-gray-100 text-gray-600 text-[10px] font-bold px-2 py-1 rounded">{opp.situation}</span>}
                          <span className="bg-gray-100 text-gray-600 text-[10px] font-bold px-2 py-1 rounded">{getStatusLabel(opp.internal_status)}</span>
                        </div>

                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-4 border-t border-gray-50">
                          <div className="flex flex-col">
                            <span className="text-[10px] text-gray-400 font-bold uppercase tracking-wider">Valor Estimado</span>
                            <span className="text-sm font-bold text-gray-900">{opp.estimated_value ? formatCurrency(opp.estimated_value) : 'Não informado'}</span>
                          </div>
                          <div className="flex flex-col">
                            <span className="text-[10px] text-gray-400 font-bold uppercase tracking-wider">Abertura</span>
                            <span className="text-sm font-bold text-gray-900">{opp.opening_date ? formatDate(opp.opening_date) : 'N/A'}</span>
                          </div>
                          <div className="flex flex-col">
                            <span className="text-[10px] text-gray-400 font-bold uppercase tracking-wider">Município Vinculado</span>
                            <span className="text-sm font-bold text-gray-900">{opp.municipality_name || 'Não vinculado'}</span>
                          </div>
                          <div className="flex flex-col">
                            <span className="text-[10px] text-gray-400 font-bold uppercase tracking-wider">Status Interno</span>
                            <span className="text-sm font-bold text-gray-900">{getStatusLabel(opp.internal_status)}</span>
                          </div>
                        </div>
                      </div>

                      <div className="flex flex-row md:flex-col gap-2 justify-end md:border-l md:border-gray-100 md:pl-6">
                        <button onClick={() => { setSelectedOpp(opp); setIsDetailModalOpen(true); }} className="flex-1 md:flex-none p-2.5 rounded-lg border border-gray-200 text-gray-500 hover:bg-gray-50 hover:text-[#0f49bd] transition-all flex items-center justify-center gap-2 text-sm font-bold">
                        ...
                        <button onClick={() => { if (opp.official_url) window.open(opp.official_url, '_blank', 'noopener,noreferrer'); else toast.error('Link oficial não disponível.'); }} className="flex-1 md:flex-none p-2.5 rounded-lg bg-[#0f49bd] text-white hover:bg-[#0a3690] transition-all flex items-center justify-center gap-2 text-sm font-bold shadow-sm">
                    ...
                    {Number(opp.match_score || 0) >= 50 && (
                      <div className="bg-blue-50/50 px-6 py-3 border-t border-blue-100 flex items-center gap-3">
                        ...
                        <button onClick={() => { setSelectedOpp(opp); setIsDetailModalOpen(true); }} className="ml-auto text-xs font-bold text-blue-700 hover:underline flex items-center">
      ...
      <Dialog open={isProposalModalOpen} onOpenChange={setIsProposalModalOpen}>
        <DialogContent className="sm:max-w-[800px] max-h-[90vh] flex flex-col">
          ...
          <DialogFooter className="flex gap-2 pt-4 border-t border-gray-100">
            <button onClick={() => setIsProposalModalOpen(false)} className="px-4 py-2 text-sm font-bold text-gray-500 hover:text-gray-700">
              Fechar
            </button>
            <button onClick={handleSaveProposal} disabled={isSavingProposal || isGeneratingProposal || !proposalContent} className="flex items-center gap-2 px-5 py-2 bg-gray-100 text-gray-700 rounded-lg font-bold text-sm hover:bg-gray-200 transition-all disabled:opacity-50">
              {isSavingProposal ? <Loader2 className="size-4 animate-spin" /> : <Save className="size-4" />}
              Salvar
            </button>
            <button onClick={handleDownloadPDF} disabled={isGeneratingProposal || !proposalContent} className="flex items-center gap-2 px-5 py-2 bg-purple-600 text-white rounded-lg font-bold text-sm hover:bg-purple-700 transition-all disabled:opacity-50 shadow-sm">
      ...
      <Dialog open={isIAInsightsModalOpen} onOpenChange={setIsIAInsightsModalOpen}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2"><Sparkles className="size-5 text-[#0f49bd]" /> IA Insights</DialogTitle>
            <DialogDescription>Resumo das oportunidades com base na aderência identificada.</DialogDescription>
          </DialogHeader>
          <div className="py-6 space-y-6">
            ...
            {iaSummary.topRecommendation && (
              <div className="space-y-3">
                <h4 className="text-sm font-bold text-gray-900 flex items-center gap-2"><CheckCircle2 className="size-4 text-green-600" /> Recomendação Prioritária</h4>
                <div className="p-4 bg-green-50 rounded-xl border border-green-100">
                  <p className="text-sm font-bold text-green-900 mb-1">{iaSummary.topRecommendation.title}</p>
                  <p className="text-xs text-green-700 leading-relaxed"><span className="font-bold">Justificativa:</span> {iaSummary.topRecommendation.match_reason || 'Sem justificativa.'}</p>
                </div>
              </div>
            )}
            <div className="space-y-3">
              <h4 className="text-sm font-bold text-gray-900 flex items-center gap-2"><AlertCircle className="size-4 text-amber-600" /> Alertas</h4>
              <ul className="space-y-2">
                <li className="text-xs text-gray-600 flex items-start gap-2"><div className="size-1.5 rounded-full bg-amber-500 mt-1.5 shrink-0" />Use "Recalcular Score" sempre que atualizar o perfil estratégico.</li>
                <li className="text-xs text-gray-600 flex items-start gap-2"><div className="size-1.5 rounded-full bg-amber-500 mt-1.5 shrink-0" />Use "Gerar Proposta" nas licitações com maior aderência ao seu perfil.</li>
              </ul>
            </div>
          </div>
          <DialogFooter>
            <button onClick={() => setIsIAInsightsModalOpen(false)} className="w-full bg-[#0f49bd] text-white py-2.5 rounded-lg font-bold text-sm hover:bg-[#0a3690] shadow-sm transition-all">Entendido</button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
          ...
          {selectedOpp && (
            <div className="py-4 space-y-6">
              <div className="space-y-2">
                ...
                <p className="text-sm text-gray-500 flex items-center gap-1"><Building2 className="size-4" /> {selectedOpp.organ_name}</p>
              </div>
              <div className="grid grid-cols-2 gap-6">
                <div>
                  <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-1">Valor Estimado</span>
                  <span className="text-sm font-bold text-gray-900">{selectedOpp.estimated_value ? formatCurrency(selectedOpp.estimated_value) : 'Não informado'}</span>
                </div>
                <div>
                ...
                <div>
                  ...
                  <span className="text-sm font-bold text-gray-900">{selectedOpp.opening_date ? formatDate(selectedOpp.opening_date) : 'N/A'}</span>
                </div>
                <div>
                  <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-1">Match IA</span>
                  <span className={cn('text-sm font-bold', Number(selectedOpp.match_score || 0) >= 90 ? 'text-green-600' : 'text-blue-600')}>{Number(selectedOpp.match_score || 0)}%</span>
                </div>
              </div>
              <div className="p-4 bg-gray-50 rounded-xl border border-gray-100">
                <h4 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Resumo Analítico</h4>
                <p className="text-sm text-gray-600 leading-relaxed italic">&quot;{selectedOpp.match_reason || 'Sem justificativa analítica disponível.'}&quot;</p>
              </div>
              <div className="p-4 bg-gray-50 rounded-xl border border-gray-100">
                <h4 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Descrição</h4>
                <p className="text-sm text-gray-700 leading-relaxed">{selectedOpp.description || 'Sem descrição detalhada disponível.'}</p>
              </div>
              <div className="flex items-center gap-3 pt-4 border-t border-gray-100">
                <button onClick={() => { if (selectedOpp.official_url) window.open(selectedOpp.official_url, '_blank'); else toast.error('Link não disponível.'); }} className="flex-1 bg-[#0f49bd] text-white py-2.5 rounded-lg font-bold text-sm hover:bg-[#0a3690] shadow-sm transition-all flex items-center justify-center gap-2">
                  <ExternalLink className="size-4" /> Ver Link Oficial
                </button>
                <button onClick={() => { setIsDetailModalOpen(false); handleGenerateProposal(selectedOpp); }} className="flex-1 bg-purple-600 text-white py-2.5 rounded-lg font-bold text-sm hover:bg-purple-700 shadow-sm transition-all flex items-center justify-center gap-2">
      ...
      <Dialog open={isFilterModalOpen} onOpenChange={setIsFilterModalOpen}>
        <DialogContent className="sm:max-w-[400px]">
          ...
          <DialogFooter className="flex gap-2">
            <button onClick={() => { setFilters({ location: '', modality: '', minScore: 0 }); toast.info('Filtros limpos.'); }} className="flex-1 px-4 py-2 text-sm font-bold text-gray-500 hover:text-gray-700 border border-gray-200 rounded-lg">Limpar</button>
