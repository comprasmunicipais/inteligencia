...
export async function GET() {
  try {
    ...
    const { data, error } = await supabase
      ...
      .select(
        'id, company_id, provider_type, oauth_provider, oauth_email, oauth_token_expires_at, oauth_status, oauth_last_error, oauth_connected_at, name, sender_name, sender_email, reply_to_email, smtp_host, smtp_port, smtp_secure, smtp_username, daily_limit, hourly_limit, is_active, last_tested_at, last_test_status, last_test_error, spf_status, dkim_status, dkim_selector, created_at, updated_at'
      )
